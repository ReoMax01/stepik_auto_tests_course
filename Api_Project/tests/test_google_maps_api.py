...
class Test_create_place():
    ...
    def test_create_new_place(self):
        ...
        print("Метод GET для проверки после метода POST")
        ...
        print("Метод GET для проверки после метода PUT")
        result_get = Google_maps_api.get_new_place(place_id)
        Checking.check_status_code(result_get, 200)
        Checking.check_json_token(result_get, ['location', 'accuracy', 'name', 'phone_number', 'address', 'types', 'website', 'language'])
        ...
        print("Метод GET для проверки после метода DELETE")
        result_get = Google_maps_api.get_new_place(place_id)
        Checking.check_status_code(result_get, 404)
        Checking.check_json_token(result_get,['msg'])
        Checking.check_json_value(result_get, 'msg', "Gett operation failed, looks like place_id  doesn't exists")
